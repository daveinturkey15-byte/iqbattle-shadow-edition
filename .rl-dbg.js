/* ============================================================
   ...
   BALANCE (2026-08-25 pass):
     win  -> RL_PAY[ctx.diff] = 140/210/290/380/470 pts: inside the takeover
             band [60%,135%] of puzzle payout 100*diff+40 at every depth and
             under the engine's takeover parity cap (100*diff+60).
     fail -> -40 pts / -12 hp on ANY red-light input (move/click/type) or a
             green timeout; guessing at a pattern beats letting the round
             expire, so timeout is never optimal.
     risk curve -> red windows lengthen with depth (+40 ms/depth, +800 cap)
             while greens shrink with tier; each green funds its own red so
             the stage always self-resolves inside timerLen.
   ============================================================ */
(function(){
'use strict';
...
var RL_PAY=[140,210,290,380,470]; /* win points by ctx.diff: keeps the cross-the-field
payout inside the takeover band [60%,135%] of 100*diff+40 at every depth
(tier saturates at 3 by round ~7, so the old 120+tier*40 curve went flat) */
var T={}; /* live handles */
...
function win(){
 if(T.done)return;T.done=true;
 const res={kind:'score',correct:true,points:T.pay,hpDelta:0,summary:'CROSSED THE FIELD'};
...
root.Stage.register({
 ...
 mount(container,ctx){
  ...
  T.pay=RL_PAY[Math.max(0,Math.min(4,(ctx.diff|0)-1))]||RL_PAY[0];
  const N=3+(ctx.tier>=2?1:0);
  const GREEN_FLOOR=2400;
  const greenBase=Math.max(GREEN_FLOOR,(ctx.timerLen*1000)*.20); /* was .22 */
  /* red exposure grows with depth (+40 ms/depth, +800 cap): the freeze-or-solve
     risk curve scales with depth. One rng call per phase, phase order, so MP
     tabs stay bit-identical. */
  var reds=[];for(var pi=0;pi<N;pi++)reds.push(1600+Math.floor(T.rng()*1100)+Math.min(800,(ctx.depth|0)*40));
  /* fairness rail: the stage MUST self-resolve inside timerLen (the engine
     injects a wrong answer on overrun — never acceptable for a perfect run).
     Scale drawn red windows together if they would overrun the green-floor
     budget; pure f(drawn values), so host and clients still agree. */
  var redBudget=Math.max(N*1200,ctx.timerLen*1000-700-N*GREEN_FLOOR);
  var redSum=0;for(pi=0;pi<N;pi++)redSum+=reds[pi];
  if(redSum>redBudget){var rk=redBudget/redSum;for(pi=0;pi<N;pi++)reds[pi]=Math.round(reds[pi]*rk)}
 ...
 T.readyEl=ready;T.readyT=setTimeout(()=>{ready.style.opacity='0'},3000);
  ...
  window.__REDLIGHT__={correctOpt:()=>T.qA,phase:()=>T.phase,phases:()=>T.phases,pay:()=>T.pay};
