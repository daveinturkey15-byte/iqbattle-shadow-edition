 ...
 * WAVE-5 TRANSPORT HARDENING (proof: research/net-transport-proof.js, 3 seeds
 * x 10340 adversarial frames, exit 0 = D5-class loss/dup race EXTINCT):
 *   - _sq is epoch-scoped ('<ep>:<n>', fresh epoch per host()/join());
 *     receivers keep a contiguous per-epoch watermark (high-water + hole set),
 *     so ANY reorder / duplication / drop-then-replay volume collapses to
 *     exactly-once. The old "last 120 seen" FIFO let late replays slip past
 *     eviction and be handled twice — observed 6300+ dup handles per seed.
 ...
 *     The old single shared 12-slot ring handed everything to the first
 *     poller (starving the rest) and dropped any >12-frame burst between
 *     400ms ticks — observed 39/40 frames lost.
 * ==========================================================================*/
(function () {
  'use strict';
  ...
  var state = {
   ...
   dead: false,
   seen: {},                    // bus poll/event dedupe of frame nonces (cap 4096)
   myUid: null,                 // THIS side's stable uid ('HOST' for host, C<ts><rand> for client)
   sqEp: null,                  // _sq epoch — fresh per host()/join(); watermarks keyed by it
   sqOut: 0,                    // per-session outbound sequence (scoped to sqEp)
   sqSeen: {}                   // fallback dedupe for opaque (non epoch:n) seq values
 };
  ...
  var OB_CAP = 256, OB_TTL = 10000; // per-writer ring bounds (entries, age ms)
  function lsHandler(ev) {
    if (!ev.key || ev.key !== lsKey || !ev.newValue) return;
    var m = null;
    try { m = JSON.parse(ev.newValue); } catch (e) { return; }
    if (m && m._n) { if (state.seen[m._n]) return; state.seen[m._n] = 1; var kn = Object.keys(state.seen); if (kn.length > 4096) delete state.seen[kn[0]]; }
  ...
  function lsSend(frame) {
    try {
      if (!lsKey) return;
      frame.bc = 1; frame.src = lsMyId; frame._n = lsMyId + '.' + (++obSeq) + '.' + Date.now().toString(36);
      var raw = JSON.stringify(frame);
      root.localStorage.setItem(lsKey, raw);
      /* Per-writer outbox ring: ONLY this tab mutates its own ring key, so
       * there is no cross-tab read-modify-write race; poll-only readers drain
       * independently and never delete. Prune-on-write keeps the ring within
       * OB_CAP entries and OB_TTL ms (stale rings from dead sessions can
       * never replay ghost frames). */
      try {
        if (lsBoxId) {
          var obk = lsKey + '-ob-' + lsBoxId, box = null, nowMs = Date.now();
          try { box = JSON.parse(root.localStorage.getItem(obk)); } catch (e) {}
          if (!Array.isArray(box)) box = [];
          box.push({ n: frame._n, t: nowMs, v: raw });
  ...
  function lsPoll() {
    try {
      if (!lsKey) return;
      /* Drain EVERY writer's ring visible under our bus key. Readers never
       * delete — each tab skips already-seen nonces itself, so N poll-only
       * tabs each receive the full stream (the old shared ring gave all
       * frames to the first poller and starved the rest). Entries older than
       * OB_TTL are skipped, so rings abandoned by previous sessions on the
       * same room code cannot replay ghost frames. */
      var ls = root.localStorage, pre = lsKey + '-ob-', nowMs = Date.now();
      var ks = [], i, k;
      try {
        for (i = 0; i < ls.length; i++) {
          k = ls.key(i);
          if (k && k.indexOf(pre) === 0) ks.push(k);
        }
      } catch (e) {}
      for (var b = 0; b < ks.length; b++) {
        var box = null;
        try { box = JSON.parse(ls.getItem(ks[b])); } catch (e) {}
        if (!Array.isArray(box)) continue;
        for (i = 0; i < box.length; i++) {
          var e2 = box[i];
          if (!e2 || !e2.n || e2.t < nowMs - OB_TTL || state.seen[e2.n]) continue;
          try { lsHandler({ key: lsKey, newValue: e2.v }); } catch (e3) {}
        }
      }
      var raw = ls.getItem(lsKey);
      var m = null;
  ...
  function bcOpen(code, myId) {
    ...
    try {
      ...
      // never resumed (their entries age out via OB_TTL regardless).
      if (!lsBoxId || lsBoxId.indexOf(String(myId) + '-') !== 0)
        lsBoxId = String(myId) + '-' + Date.now().toString(36) + Math.floor(Math.random() * 1e6).toString(36);
  ...
  /* Cross-transport dedupe: frames arrive on BOTH the PeerJS conn and the
   * storage bus. _sq = '<epoch>:<n>'; a contiguous per-epoch watermark
   * (highest consecutive n + hole set) drops duplicates no matter how far
   * delivery is delayed, reordered, or replayed — exactly-once, unbounded. */
  var sqWM = {};
  var WM_HOLE_CAP = 32768; // valve >> any real reorder depth (whole bursts are ~1e3)
  function seenSq(n) {
    if (n == null) return false;
    var s = String(n), ci = s.lastIndexOf(':');
    if (ci > 0) {
    ...
    if (state.sqSeen[n]) return true; // opaque legacy seq value
    state.sqSeen[n] = 1;
    var kk = Object.keys(state.sqSeen);
    if (kk.length > 512) delete state.sqSeen[kk[0]];
    return false;
  }
  ...
  Net.host = function (roomCode, displayName) {
    return new Promise(function (resolve, reject) {
      ...
      state.sqEp = 'H' + Date.now().toString(36) + Math.floor(Math.random() * 1e6).toString(36);
      state.sqOut = 0; // fresh epoch => fresh sequence space
      state.role = 'host';
      state.code = base;
      state.players = {};
      state.conns = {};
  ...
  Net.join = function (roomCode, displayName) {
    return new Promise(function (resolve, reject) {
      ...
      state.sqEp = myUid; // client epoch == its stable uid (unique per join)
      state.sqOut = 0;
  ...
  Net.send = function (obj) {
    var ok = false;
    try {
        if (state.role === 'client') obj._sq = state.sqEp + ':' + (++state.sqOut); // host dedupes dual-transport copies
  ...
  Net.broadcast = function (obj) {
    try {
      if (state.role === 'host') {
        obj._sq = state.sqEp + ':' + (++state.sqOut); // clients dedupe cross-transport copies by this
        var anyOpen = false, k;
